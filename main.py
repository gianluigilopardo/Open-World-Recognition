...
# model
model = ResNet.resnet32(num_classes=100).to(params.DEVICE)
...
for task in range(0, 100, params.TASK_SIZE):
    ...
    train_loader = DataLoader(train_subset, num_workers=params.NUM_WORKERS, batch_size=params.BATCH_SIZE, shuffle=True)
    test_loader = DataLoader(test_subset, num_workers=params.NUM_WORKERS, batch_size=params.BATCH_SIZE, shuffle=True)
    ...
    for img, lbl, _ in train_loader:
        ...
        preds, mean = model.classify(img, exemplars, ICaRL, task, train_dataset, mean)
    ...
    for img, lbl, _ in test_loader:
        img = img.float().to(params.DEVICE)
        preds, _ = model.classify(img, exemplars, model, task, train_dataset, mean)
        preds = preds.to(params.DEVICE)
        labels = utils.mapFunction(lbl, col).to(params.DEVICE)
